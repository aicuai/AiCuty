# Elena Bloom — ちびループダンス（MiniMax H3）

## seed 330

| | |
|---|---|
| ファイル | `loop-330.mp4`（緑背景・608×800・124 フレーム・24fps）／ `loop-330.webm`（クロマキー抜き・alpha） |
| モデル | MiniMax-H3 Ref2VA（pruned_int8_convrot）20 step、ComfyUI 標準ノード |
| 参照 | `elena-chibi-green.png`（comfy-aicu `infra/bench/video/refs/`） |
| seed | 330（誕生日） |
| 生成 | NVIDIA RTX 4000 Ada 20GB × 1、6 分、comfy-aicu Actions run 33711633387 |
| 検査 | 上余白 27px（全フレーム最小）／ 最初と最後のフレームの差 6.8（255 中） |

### プロンプト（H3-Context-IR）

```
subject_definitions:
<Subject 1> is a chibi (super-deformed, 2.5 heads tall) version of Elena Bloom: a chibi girl with long pink twin-tails tied with pink rose ornaments, a gold hair pin in her bangs, large pink-magenta eyes, a white-and-pink idol dress with gold trim, a pink bow at the waist, white boots with gold trim.
