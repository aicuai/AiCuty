...
| | |
|---|---|
| ファイル | `loop-20250330.mp4`（緑背景・124 フレーム・24fps）／ `loop-20250330.gif`（プレビュー）／ `loop-20250330.webm`（クロマキー抜き・alpha） |
...
| seed | 20250330 |
| 生成 | NVIDIA RTX 4000 Ada 20GB × 1、6 分、comfy-aicu Actions run 33730023618 |
| 検査 | 上余白 126px（全フレーム最小）／ 最初と最後のフレームの差 3.9（255 中） |

### プロンプト（H3-Context-IR）

```
subject_definitions:
<Subject 1> is a chibi (super-deformed, 2.5 heads tall) version of Elena Bloom: a chibi girl with fluffy pink twin-tails tied high with big pastel-pink ribbons and rose flower hair clips, choppy bangs with a gold hair pin, large pink-magenta eyes, a gentle smile, a goddess-inspired idol stage outfit in pastel pink and white with gold accents: an off-shoulder mini dress with a cropped white bolero with sheer sleeves, a layered skirt (bright pink outer, white chiffon inner) to mid-thigh, a slim gold belt with a pink gem and a pink bow, fingerless lace gloves with tiny pink bows, white socks and white ankle boots with gold trim. The bolero, lace gloves and ankle boots stay as in <Image 1>.
