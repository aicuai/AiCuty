...
納品区分は **Chibi / Standard / Photo** の3系統で、設定の正本かつ生成AIのリファレンスになるのは **Standard** です。
**別のマシンで作業を再開する場合**は [AGENTS.md](AGENTS.md) の「別のマシンで作業を再開するとき」を先に読んでください（エージェントの記憶とスキルは git で移動しません）。
...
- **Marsha の一人称はカタカナ「アタシ」**が正式表記（ひらがな「あたし」は誤り）
- **ボイスサンプルは説明的な文面に直さない**（声の見本として成立させることを優先）
