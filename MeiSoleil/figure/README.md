# Mei Soleil — 公式フィギュアのターンテーブル（MiniMax H3）

## seed 721（誕生日）

| | |
|---|---|
| ファイル | `turntable-721.mp4`（704×704・124 フレーム・24fps・5.2 秒・灰背景）／ `turntable-721.gif`（プレビュー） |
| 参照 | `img/figure/mei.png`（このリポジトリの公式フィギュア風レンダー） |
...
```
subject_definitions:
<Subject 1> is a collectible figure of Mei Soleil (a chibi girl with vibrant golden-yellow hair in a high side ponytail with soft outward curls tied with a yellow ribbon, short side bangs, a star-shaped yellow hairpin, amber-brown eyes, freckles, a bright cheerful smile, a sun-yellow cropped tech jacket with silver accents over a white crop top, a pleated asymmetrical vivid-yellow mini skirt layered over pale-yellow utility shorts, a yellow harness belt with a carabiner charm, white socks and yellow high-top sneakers with glowing soles), standing on a round white display base, exactly as shown in <Image 1>: same sculpt, same paint, same pose, same base.
