...
| | |
|---|---|
| ファイル | `loop-414.mp4`（緑背景・608×800・124 フレーム・24fps）／ `loop-414.webm`（クロマキー抜き・alpha） |
...
| seed | 414（誕生日） |
| 生成 | NVIDIA RTX 4000 Ada 20GB × 1、6 分、comfy-aicu Actions run 33715821364 |
| 検査 | 上余白 107px（全フレーム最小）／ 最初と最後のフレームの差 2.7（255 中） |
