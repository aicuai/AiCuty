# Nao Verde — 公式フィギュアのターンテーブル（MiniMax H3）

## seed 505（誕生日）

| | |
|---|---|
| ファイル | `turntable-505.mp4`（704×704・124 フレーム・24fps・5.2 秒・灰背景）／ `turntable-505.gif`（プレビュー） |
| 参照 | `img/figure/nao.png`（このリポジトリの公式フィギュア風レンダー） |
...
```
subject_definitions:
<Subject 1> is a collectible figure of Nao Verde (a chibi boy (androgynous but male) with dark green tousled short hair, emerald green eyes, a black sleeveless turtleneck under an oversized olive bomber jacket, black cargo pants with straps, black lace-up boots), standing on a round white display base, exactly as shown in <Image 1>: same sculpt, same paint, same pose, same base.
