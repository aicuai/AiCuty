# Nao Verde — ちびループダンス（MiniMax H3）

## seed 505

| | |
|---|---|
| ファイル | `loop-505.mp4`（緑背景・608×800・124 フレーム・24fps）／ `loop-505.webm`（クロマキー抜き・alpha） |
| モデル | MiniMax-H3 Ref2VA（pruned_int8_convrot）20 step、ComfyUI 標準ノード |
| 参照 | `nao-chibi-green.png`（comfy-aicu `infra/bench/video/refs/`） |
| seed | 505（誕生日） |
| 生成 | NVIDIA RTX 4000 Ada 20GB × 1、6 分、comfy-aicu Actions run 33714697090 |
| 検査 | 上余白 107px（全フレーム最小）／ 最初と最後のフレームの差 2.1（255 中） |

### プロンプト（H3-Context-IR）

```
subject_definitions:
<Subject 1> is a chibi (super-deformed, 2.5 heads tall) version of Nao Verde: a chibi boy (androgynous but male) with dark green tousled short hair, emerald green eyes, a black sleeveless turtleneck under an oversized olive bomber jacket, black cargo pants with straps, black lace-up boots.
